...
To add the repo to helm with the name `MisoAI`:

```bash
$ helm repo add MisoAI https://MisoAI.github.io/charts
```
...
```bash
$ helm install --name my-release MisoAI/pypiserver
```
...
```bash
$ helm upgrade my-release MisoAI/pypiserver
```
...
| `image.tag`                        | Container image name                                                                     | `v2.0.1`                |
